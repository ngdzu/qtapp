...
**Purpose:** Domain aggregate managing telemetry data collection and batch preparation for transmission to central server. Enforces business rules such as "all vitals in batch must be from same patient" and "batch must be signed before transmission."

**Responsibilities:**
- Collect vital records into batches for efficient transmission
- Collect alarm snapshots into batches
- Enforce batch size limits (prevent oversized payloads)
- Sign batches with digital signature (HMAC-SHA256 or ECDSA)
- Validate batch integrity before transmission
- Associate batch with patient MRN (if patient admitted)
- Generate unique batch IDs (UUIDs) and timestamps

**Layer:** Domain Layer

**Module:** `z-monitor/src/domain/monitoring/TelemetryBatch.h`

**Dependencies:**
- VitalRecord - Individual vital sign measurement
- AlarmSnapshot - Alarm state snapshot
- Batch signing/validation utilities
