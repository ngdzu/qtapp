---
doc_id: DOC-COMP-003
title: DeviceAggregate
version: v1.0
category: Component
subcategory: Domain Layer / Provisioning
status: Draft
...
**Purpose:** Domain aggregate managing device provisioning state, credential lifecycle, and firmware metadata. Enforces business rules such as "device can only be provisioned once" and "credentials must be valid before provisioning."

**Responsibilities:**
- Manage device provisioning state (Unprovisioned, Provisioning, Provisioned, Failed)
- Store device credentials (certificates, private keys)
- Track server URL and configuration
- Manage credential rotation for security
- Raise domain events (ProvisioningCompleted, ProvisioningFailed)
- Enforce provisioning business rules

**Layer:** Domain Layer

**Module:** `z-monitor/src/domain/provisioning/DeviceAggregate.h`

**Dependencies:**
- Result<T> - Error handling type
- ProvisioningStatus enum - Device provisioning state
