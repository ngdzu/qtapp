...
version: v1.0
category: Component
subcategory: Domain Layer / Provisioning (Value Object)
status: Draft
...
# DOC-COMP-006: DeviceSnapshot

## 1. Overview

**Purpose:** Immutable value object representing a snapshot of device state (device ID, firmware version, last sync time, battery status). Value objects are defined by their attributes.

**Responsibilities:**
- Capture device state at a point in time
- Provide immutable device metadata
- Support device status reporting
- Enable device state history tracking

**Layer:** Domain Layer (Value Object)

**Module:** `z-monitor/src/domain/provisioning/` (to be implemented)

**Key Attributes:**
- `deviceId` - Device serial number or identifier
- `firmwareVersion` - Current firmware version
- `lastSyncTimeMs` - Last successful sync with central server (Unix milliseconds)
