...
version: v1.0
category: Component
subcategory: Domain Layer / Admission (Value Object)
status: Draft
...
**Purpose:** Immutable value object representing patient demographic information (MRN, name, DOB, sex, allergies). Value objects are defined by their attributes and have no identity.

**Responsibilities:**
- Encapsulate patient demographic information
- Provide immutable patient identity data
- Support equality comparison (two patients with same MRN are equal)
- Ensure thread-safe sharing (all members are const)

**Layer:** Domain Layer (Value Object)

**Module:** `z-monitor/src/domain/admission/PatientIdentity.h`

**Key Attributes:**
- `mrn` (const std::string) - Medical Record Number (primary identifier)
- `name` (const std::string) - Patient full name
- `dateOfBirthMs` (const int64_t) - Date of birth in Unix milliseconds
- `sex` (const std::string) - Patient sex ("M", "F", "Other", "Unknown")
- `allergies` (const std::vector<std::string>) - List of known allergies
