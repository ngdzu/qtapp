...
def _canonical(text: str) -> str:
    """Return canonical form of generated content excluding ephemeral timestamp lines."""
    lines = []
    for line in text.splitlines():
        if line.startswith(' * Generated: ') or line.startswith('-- Generated: '):
            continue
        lines.append(line.rstrip())  # trim trailing whitespace for stable compare
    return '\n'.join(lines)

def _maybe_write(path: str, new_content: str) -> bool:
    """Write file only if canonical (meaningful) content changed. Returns True if written."""
    os.makedirs(os.path.dirname(path), exist_ok=True)
    if os.path.exists(path):
        with open(path, 'r') as f:
            old_content = f.read()
        if _canonical(old_content) == _canonical(new_content):
            print(f"⚠️ Unchanged (skipped write): {path}")
            return False
...
def _schema_hash(schema_file: Path) -> str:
    try:
...
def generate_schema_header(schema: Dict[str, Any], output_file: str, schema_file: Path):
    """Generate SchemaInfo.h C++ header file (skip rewrite if only timestamp differs)."""

    timestamp = datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')
    version = schema.get('version', '1.0.0')
    source_hash = _schema_hash(schema_file)
 ...
 * Schema Source Hash: {source_hash}
 */
...
"""
    ...
    _maybe_write(output_file, header)

def generate_ddl(schema: Dict[str, Any], output_dir: str, schema_file: Path):
    """Generate SQL DDL files (skip rewrite if only timestamp differs)."""
...
"""
    ...
    # Write CREATE TABLES (conditional)
    os.makedirs(output_dir, exist_ok=True)
    create_tables_file = os.path.join(output_dir, "create_tables.sql")
    _maybe_write(create_tables_file, create_tables_sql)
...
"""
    ...
    _maybe_write(create_indices_file, create_indices_sql)
...
def main():
    ...
    generate_schema_header(schema, str(header_output), schema_file)
    
    print(f"🔨 Generating SQL DDL...")
    generate_ddl(schema, str(ddl_output_dir), schema_file)
