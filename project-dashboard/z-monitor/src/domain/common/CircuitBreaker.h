...
namespace zmon
{
    ...
    public:
         ...
         */
        CircuitBreaker()
            : m_state(CircuitState::Closed), m_failureCount(0), m_successCount(0), m_failureThreshold(5), m_resetTimeoutSeconds(60), m_halfOpenMaxRequests(3), m_lastFailureTime(std::chrono::steady_clock::now())
        {
        }
        ...
        CircuitBreaker(int failureThreshold, int resetTimeoutSeconds, int halfOpenMaxRequests)
            : m_state(CircuitState::Closed), m_failureCount(0), m_successCount(0), m_failureThreshold(failureThreshold), m_resetTimeoutSeconds(resetTimeoutSeconds), m_halfOpenMaxRequests(halfOpenMaxRequests), m_lastFailureTime(std::chrono::steady_clock::now())
        {
        }
        ...
        {
            using ResultType = std::decay_t<decltype(operation())>;
            using T = typename result_value_type<ResultType>::type;
                        ...
                        {{"failureCount", std::to_string(m_failureCount.load())},
                         {"lastFailureTime", std::to_string(
                                                 std::chrono::duration_cast<std::chrono::seconds>(
                                                     m_lastFailureTime.time_since_epoch())
                                                     .count())}}));
                }
            }
