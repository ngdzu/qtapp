...
namespace zmon
{
    ...
    public:
        ...
        /// Returns the value if ok, otherwise returns the provided fallback (const ref).
        [[nodiscard]] const T &valueOr(const T &fallback) const
        {
            return m_value.has_value() ? m_value.value() : fallback;
        }

        /// Returns the value if ok, otherwise returns the provided fallback (rvalue).
        [[nodiscard]] T valueOr(T &&fallback) const
        {
            return m_value.has_value() ? m_value.value() : std::move(fallback);
        }
