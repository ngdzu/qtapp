...
namespace zmon
{
    ...
    SettingsController::SettingsController(IActionLogRepository *actionLogRepo, SecurityService *securityService, QObject *parent)
        : QObject(parent), m_actionLogRepo(actionLogRepo), m_settingsManager(&SettingsManager::instance()), m_securityService(securityService), m_deviceLabel(m_settingsManager->deviceLabel()), m_measurementUnit(m_settingsManager->measurementUnit()), m_serverUrl(m_settingsManager->serverUrl()), m_useMockServer(m_settingsManager->useMockServer())
    {
    ...
    void SettingsController::setDeviceLabel(const QString &deviceLabel)
    {
        ...
        {
            // TODO: Permission check (requires correct Permission enum)
    ...
    void SettingsController::setMeasurementUnit(const QString &unit)
    {
        if (m_measurementUnit != unit)
        {
            ...
            {
                emit settingsChangeFailed("Invalid measurement unit");
                return;
            }
            // TODO: Permission check (requires correct Permission enum)
    ...
    void SettingsController::setServerUrl(const QString &url)
    {
        if (m_serverUrl != url)
        {
            ...
            {
                emit settingsChangeFailed("Server URL must start with http:// or https://");
                return;
            }
            // TODO: Permission check (requires correct Permission enum)
