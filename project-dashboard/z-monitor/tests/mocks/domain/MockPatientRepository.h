...
public:
    ...
    std::shared_ptr<PatientAggregate> findByMrn(const std::string& mrn) override;
    Result<void> save(const PatientAggregate& patient) override;
    std::vector<std::string> getAdmissionHistory(const std::string& mrn) override;
    std::vector<std::shared_ptr<PatientAggregate>> findAll() override;
