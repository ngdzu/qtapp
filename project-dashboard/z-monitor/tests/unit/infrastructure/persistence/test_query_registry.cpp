...
protected:
    ...
    void createTestSchema() {
        ...
        if (!query.exec(createPatientsTableSql)) {
            FAIL() << "Failed to create patients table: " << query.lastError().text();
        }
        ...
        if (!query.exec(createActionLogTableSql)) {
            FAIL() << "Failed to create action_log table: " << query.lastError().text();
        }
    }
...
TEST_F(QueryRegistryTest, PatientFindByMrn_Works) {
    ...
    ASSERT_TRUE(insertQuery.exec()) << "Failed to insert test patient: " << insertQuery.lastError().text();
    ...
    ASSERT_TRUE(findQuery.exec()) << "Query execution failed: " << findQuery.lastError().text();
...
TEST_F(QueryRegistryTest, PatientInsert_Works) {
    ...
    ASSERT_TRUE(insertQuery.exec()) << "Insert failed: " << insertQuery.lastError().text();
...
TEST_F(QueryRegistryTest, PatientCheckExists_Works) {
    ...
    ASSERT_TRUE(checkQuery.exec()) << "Query execution failed: " << checkQuery.lastError().text();
...
TEST_F(QueryRegistryTest, PatientFindAll_Works) {
    ...
    ASSERT_TRUE(findAllQuery.exec()) << "Query execution failed: " << findAllQuery.lastError().text();
