...
protected:
    void SetUp() override
    {
        m_backend = std::make_unique<ZMonitor::Infrastructure::Logging::MockLogBackend>();
        m_logService = std::make_unique<zmon::LogService>(m_backend.release());
    ...
    std::unique_ptr<ZMonitor::Infrastructure::Logging::MockLogBackend> m_backend;
};
...
{
    auto result = m_logService->initialize("/tmp", "test-log");
...
{
    m_logService->initialize("/tmp", "test-log");
    m_logService->setLogLevel(zmon::LogLevel::Warning);
...
TEST_F(LogServiceTest, RecentLogs)
{
    ...
    // Get recent logs
    QList<zmon::LogEntry> recentLogs = m_logService->recentLogs();
...
TEST_F(LogServiceTest, Configuration)
{
    ...
    m_logService->setLogLevel(zmon::LogLevel::Debug);
    EXPECT_EQ(m_logService->logLevel(), zmon::LogLevel::Debug);
...
TEST_F(LogServiceTest, LogEntryAddedSignal)
{
    ...
    QSignalSpy spy(m_logService.get(), &zmon::LogService::logEntryAdded);
