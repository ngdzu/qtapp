...
protected:
    ...
    std::unique_ptr<CustomBackend> m_backend;
};
...
TEST_F(CustomBackendTest, InitializeSuccess) {
    m_backend = std::make_unique<CustomBackend>();
    bool result = m_backend->initialize(m_tempDir->path(), "test-log");
    EXPECT_TRUE(result);
...
TEST_F(CustomBackendTest, InitializeEmptyFilename) {
    m_backend = std::make_unique<CustomBackend>();
    bool result = m_backend->initialize(m_tempDir->path(), "");
    EXPECT_FALSE(result);
}
