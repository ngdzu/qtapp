...
protected:
    void SetUp() override {
        ...
        // Create mock objects
        m_mockTelemetryServer = new MockTelemetryServer(this);
        m_mockPatientLookupService = new MockPatientLookupService(this);
...
TEST_F(AlarmManagerTest, MockTelemetryServer_SendTelemetry_Success) {
    ...
    EXPECT_EQ(m_mockTelemetryServer->telemetrySendCount(), 1);
    EXPECT_EQ(sentSpy.count(), 1);
    
    QList<TelemetryData> sent = m_mockTelemetryServer->sentTelemetry();
...
TEST_F(AlarmManagerTest, MockTelemetryServer_SendTelemetry_Failure) {
    // Arrange
    m_mockTelemetryServer->setSimulateFailures(true);
    m_mockTelemetryServer->setFailureError("Network error");
    ...
    EXPECT_EQ(response.message, "Network error");
