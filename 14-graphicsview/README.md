...
From the **lesson directory** (`14-graphicsview`):

```bash
docker build -t qtapp-lesson14:latest .
```
...
```bash
docker run --rm -e DISPLAY=host.docker.internal:0 -e QT_LOGGING_RULES="*.debug=false;qt.qpa.*=false" qtapp-lesson14:latest
```
...
A graphics scene with movable shapes:
- Blue rectangle
- Red circle
- Green triangle

All shapes can be selected and dragged around the scene.
