...
This lesson demonstrates custom item delegates for Model/View with progress bars and combo box editors.
...
From the **lesson directory** (`10-item-delegates`):

```bash
docker build -t qtapp-lesson10:latest .
```
...
```bash
docker run --rm -e DISPLAY=host.docker.internal:0 -e QT_LOGGING_RULES="*.debug=false;qt.qpa.*=false" qtapp-lesson10:latest
```
...
A window displaying a table with custom item delegates:
