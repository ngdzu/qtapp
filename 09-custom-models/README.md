...
This lesson demonstrates custom QAbstractTableModel implementation with Task data structure.
...
From the **lesson directory** (`09-custom-models`):

```bash
docker build -t qtapp-lesson09:latest .
```
...
```bash
docker run --rm -e DISPLAY=host.docker.internal:0 -e QT_LOGGING_RULES="*.debug=false;qt.qpa.*=false" qtapp-lesson09:latest
```
...
cmake ..
cmake --build .
./lesson09-custom-models
```

## What You'll See

A window displaying a custom task management table with:
